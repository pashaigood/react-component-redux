...
| lint:tools | Runs ESLint on build related JS files. (eslint-loader lints examples files via webpack when `npm start` is run) |
...
Webpack serves your app in memory when you run `npm start`. No physical files are written. However, the web root is /examples, so you can reference files under /examples in index.html. When the app is built using `npm run build`, physical files are written to /dist and the app is served from /dist.
...
Nope. Redux is useful for applications with more complex data flows. If your app is simple, Redux is overkill. Remove Redux like this:
 ...
 4. Call render on the new top level component you created in step 3 in examples/index.js.

### How do I remove React Router?
 1. Uninstall React Router and routing related packages: `npm uninstall --save react-router`
 2. Delete the following files: `examples/routes.js`
 3. Remove `import { Link, IndexLink } from 'react-router';` from top of `examples/components/App.js`, add a reference to `examples/components/FuelSavingsForm.js`, and replace body of (implicit) render with this: `<FuelSavingsPage />`.
...
```
<img examples={require('./examples/images/myImage.jpg')} />
